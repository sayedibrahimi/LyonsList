...
export async function hostImage(
  ...
  try {
    ...
    const endpointUrl: string = `https://${DO_SPACE_REGION}.digitaloceanspaces.com`;
    ...
    // Execute the command
    const uploadCommand: PutObjectCommand = new PutObjectCommand(uploadParams);
    await s3Client.send(uploadCommand);

    // Construct the correct URL for the uploaded object
    // Format: https://<bucket>.<region>.digitaloceanspaces.com/<key>
    const imageUrl: string = `https://${DO_SPACE_NAME}.${DO_SPACE_REGION}.digitaloceanspaces.com/${fileName}`;
